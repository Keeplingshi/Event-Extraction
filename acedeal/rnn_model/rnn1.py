...
weights = {
    ...
    'output': tf.Variable(tf.random_normal([2*nHidden+nInput-23, nClasses]))
}
...
# 
    word_vec = tf.slice(x, [0, 0, 0], [-1, -1, 200])[0]
 
    x_front = tf.split(1, 1, word_vec)
    
    gru_fw_cell = tf.nn.rnn_cell.GRUCell(nHidden)
    gru_bw_cell = tf.nn.rnn_cell.GRUCell(nHidden)

#     gru_fw_cell = tf.nn.rnn_cell.MultiRNNCell([gru_fw_cell] * 4)
#     gru_bw_cell = tf.nn.rnn_cell.MultiRNNCell([gru_bw_cell] * 4)

    outputs, _, _ = tf.nn.bidirectional_rnn(gru_fw_cell, gru_bw_cell, x_front,
                                            dtype=tf.float32)

#     with tf.variable_scope('gru_fw_cell'):
#         gru_fw_cell = tf.nn.rnn_cell.GRUCell(nHidden)
#         fw_output=tf.nn.rnn(gru_fw_cell, x_front,dtype=tf.float32)[-1]
#     
#     with tf.variable_scope('gru_bw_cell'):
#         gru_bw_cell = tf.nn.rnn_cell.GRUCell(nHidden)
#         bw_output=tf.nn.rnn(gru_bw_cell, x_front,dtype=tf.float32)[-1]
    
    right_word_left = tf.concat(1, [outputs[-1],x_front[0]])
#     # batch_size*400
#     right_word_left = tf.concat(1, [right_word_left,bw_output])
#     
    results = tf.matmul(right_word_left, weights['output']) + biases['output']
...
with tf.Session() as sess:
    ...
    while k < training_iters:
        ...
        x_front=sess.run(pred, feed_dict={x: batch_xs})
        
#         print(right_word_left)
#         print(len(right_word_left))
#         print(len(right_word_left[0]))
#         print(len(right_word_left[0][0]))
#         
#         print('------------------------------------------------')
...
#         print(len(x_back))
#         print(len(x_back[0]))
#         print(len(x_back[0][0]))
#         print(len(x_back[0][0][0]))
#         
#         sys.exit()
