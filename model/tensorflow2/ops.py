...
def conv2d(x, filter_num, filter_size,filter_length,stddev=0.1,active_func="tanh", name="conv2d"):
...
def official_batch_norm_layer(inputs,channels,phase_train,type=False,scope=None):
    return tf.cond(tf.cast(phase_train,dtype=bool),
                   lambda: official_batch_norm(inputs,channels,type,is_training=True, scale=True,updates_collections=None, scope=scope),
                   lambda: official_batch_norm(inputs,channels,type,is_training=False, scale=True,updates_collections=None, scope=scope, reuse=True))
...
def batch_norm(x, size,name_scope, training, epsilon=1e-3, decay=0.999,train=True):
    ...
    with tf.variable_scope(name_scope):
        ...
        return tf.cond(tf.cast(training,dtype=bool), batch_statistics, population_statistics)
