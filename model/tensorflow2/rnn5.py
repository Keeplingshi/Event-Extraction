...
class Model:
    def __init__(self, args):
        ...
        self.cnn_output=tf.unpack(tf.reshape(tf.transpose(self.cnn_output,[1,0,2,3]), [args.sentence_length, args.batch_size,args.feature_maps]))
        ...
        output, _,_ = tf.nn.bidirectional_rnn(fw_cell, bw_cell,self.cnn_output,dtype=tf.float32, sequence_length=self.length)
...
def train(args):
    saver_path="./data/saver/checkpointrnn5_4.data"
    ...
    with tf.Session() as sess:
        ...
        for e in range(args.epoch):
            for ptr in range(0, len(train_inp), args.batch_size):
                ...
                # test=sess.run(model.cnn_output, {model.input_data: batch_xs, model.output_data: batch_ys})
                # print(np.array(test).shape)
                # test=sess.run(model.length, {model.input_data: batch_xs, model.output_data: batch_ys})
                # print(np.array(test).shape)
                # test=sess.run(model.lstm_output, {model.input_data: batch_xs, model.output_data: batch_ys})
                # print(np.array(test).shape)
                # test=sess.run(model.prediction, {model.input_data: batch_xs, model.output_data: batch_ys})
                # print(np.array(test).shape)
                # x = sess.run(model.x, {model.input_data: batch_xs, model.output_data: batch_ys})
