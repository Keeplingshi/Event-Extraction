    ...
    with tf.Session() as sess:
        ...
        for e in range(args.epoch):
            for ptr in range(0, len(train_inp), args.batch_size):
                result=sess.run(model.output, {model.input_data: train_inp[ptr:ptr + args.batch_size]
                    ,model.output_data: train_out[ptr:ptr + args.batch_size]})
                x=sess.run(model.input_data, {model.input_data: train_inp[ptr:ptr + args.batch_size]
                    ,model.output_data: train_out[ptr:ptr + args.batch_size]})
