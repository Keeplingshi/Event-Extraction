...
#             Y_train2.append(j)
    ...
    Y_train = np.asarray([np_utils.to_categorical(j,35) for j in Y_train1])
...
#     sys.exit()
    ...
    model.add(TimeDistributed(Dense(35,activation=timeDistributed_dense_activation))) 
    ...
    open('./chACEdata/class_event_my_model_architecture2.json','w').write(json_string) 

    model.save_weights('./chACEdata/class_event_my_model_weights2.h5') 
...
def lstm_test():
    ...
    X_train,X_test,Y_train,Y_test=pickle.load(data_f)
    model = model_from_json(open('./chACEdata/class_event_my_model_architecture2.json').read()) 
    model.load_weights('./chACEdata/class_event_my_model_weights2.h5')
    ...
    log_f=open('./chACEdata/class_event.log','a',encoding='utf8')
    log_f.write('\n')
